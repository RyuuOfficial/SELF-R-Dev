...
// SC INI DARI Xfarr dan Teamnya
// TAPI GW RECODE SAMPE JADI SEPERTI INI
// DAN SEKARANG SC NYA GW AMBIL ALIH
// SELAMAT MENGGUNAKAN SCRIPTURE 
const
	{
		WAConnection,
		MessageType,
...
offline = false
targetpc = '0'
ownerNumber = ["6282316153393@s.whatsapp.net","6282316153393@s.whatsapp.net","6282316153393@s.whatsapp.net","6282316153393@s.whatsapp.net","6282316153393@s.whatsapp.net","6282316153393@s.whatsapp.net"]
fake = 'RyuuOfficial'
fake1 = 'R - Development'
numbernye = '0'
waktu = '-'
...
/////*** BUKAN GW YANG ENC TAPI SI TEAM Xfarr ***\\\\\
...
var ucapanWaktu = 'Selamat pagi 🌅'                                       }
//=================================================//
module.exports = rdev = async (rdev, dev) => {
	try {
		...
		const { text, extendedText, contact, location, listMessage, buttonMessage, liveLocation, image, video, sticker, document, audio, product } = MessageType
		...
		const botNumber = rdev.user.jid
		const botNumberss = rdev.user.jid + '@c.us'
		const isGroup = from.endsWith('@g.us')
		const sender = dev.key.fromMe ? rdev.user.jid : isGroup ? dev.participant : dev.key.remoteJid
		const isOwner = ownerNumber.includes(sender)
		const totalchat = await rdev.chats.all()
		const groupMetadata = isGroup ? await rdev.groupMetadata(from) : ''
        ...
        const conts = dev.key.fromMe ? rdev.user.jid : rdev.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = dev.key.fromMe ? rdev.user.name : conts.notify || conts.vname || conts.name || '-'
        ...
        //FAKEREPLY PRODUCT
            ...
            //FAKEREPLY VIDEO
            const fvideo = {
                  key: {fromMe: false,participant: `6282316153393@s.whatsapp.net`, ...(from ? { remoteJid: "6282316153393-1613049930@g.us" } : {}) },message: { "videoMessage": { "title":"hallo bang","h": `Hmm`,'seconds': '-99999', 'caption': `${fake}`,'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}}
            //FAKEREPLY GROUPINVITE
            const fgc = {
                  key: {"fromMe": false,"participant": "0@s.whatsapp.net","remoteJid": "0@s.whatsapp.net"},"message": {"groupInviteMessage": {"groupJid": "6282316153393-1616169743@g.us","inviteCode": "mememteeeekkeke","groupName": "P", "caption": `${fake}`, 'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}}
            //FAKEREPLY GIF
            const fgif = {
                  key: {fromMe: false,participant: `6282316153393@s.whatsapp.net`, ...(from ? { remoteJid: "6282316153393-1613049930@g.us" } : {}) },message: { "videoMessage": { "title":"hallo bang","h": `Hmm`,'seconds': '99999', 'gifPlayback': 'true', 'caption': `${fake}`,'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}} 
            
        const reply = (text) => {
                rdev.sendMessage(from, text, MessageType.text, {contextInfo:{forwardingScore: 508, isForwarded: true}, quoted: dev, sendEphemeral: true ,thumbnail: fs.readFileSync(`./stik/thumb.jpeg`)})
        }
        const sendMess = (hehe, teks) => {
            rdev.sendMessage(hehe, teks, text)
        }
        const mentions = (teks, memberr, id) => {
            (id == null || id == undefined || id == false) ? rdev.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : rdev.sendMessage(from, teks.trim(), extendedText, { quoted: dev, contextInfo: { "mentionedJid": memberr } })
        }
        const fakestatus = (teks) => {
            rdev.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
        ...
        const fakethumb = (teks, yes) => {
            rdev.sendMessage(from, teks, image, {thumbnail:fs.readFileSync('./stik/fake.jpeg'),quoted:dev,caption:yes})
        }
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        rdev.sendMessage(to, media, MessageType.sticker,{quoted:dev})
        ...
        const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    rdev.sendMessage(to, media, type, { quoted: dev, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
            ...
            var chats = await rdev.chats.array.filter(v => v.jid.endsWith('g.us'))
            chats.map( async ({ jid }) => {
            await rdev.chatRead(jid)
            })
            //AUTO RESPON
      for (let i = 0; i < commandsDB.length ; i++) {
      if (budy == commandsDB[i].pesan) {
      rdev.sendMessage(from, commandsDB[i].balasan, text, {quoted: dev})
      }
      }
            /////***𝙁𝙐𝙉𝙏𝙄𝙊𝙉 𝘼𝙉𝙏𝙄𝙇𝙄𝙉𝙆 𝙂𝙍𝙐𝙋***\\\\\
				...
				setTimeout(() => {
				rdev.groupRemove(from, [kic]).catch((e) => { reply(`BOT HARUS JADI ADMIN`) })
				}, 0)
			    }
			/////***𝙁𝙐𝙉𝙏𝙄𝙊𝙉 𝘼𝙉𝙏𝙄 𝙑𝙄𝙍𝙐𝙎***\\\\\
    if (budy.length > 3500) {
...
//FUNCTION
	...
	rdev.sendMessage(from, buffer, sticker, {quoted:dev })
	}
	
	if (audionye.includes(messagesC)){
	namastc = messagesC
	buffer = fs.readFileSync(`./temp/audio/${namastc}.mp3`)
	rdev.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', quoted: dev, ptt: true })
	}
	
	if (imagenye.includes(messagesC)){
	namastc = messagesC
	buffer = fs.readFileSync(`./temp/foto/${namastc}jpeg`)
	rdev.sendMessage(from, buffer, image, {quoted : dev})
	}
	
	if (videonye.includes(messagesC)){
	namastc = messagesC
	buffer = fs.readFileSync(`./temp/video/${namastc}.mp4`)
	rdev.sendMessage(from, buffer, video, {quoted : dev})
	}
...
switch (command) {
    ...
    teks = `${kyun(run)}`
    try {
pic = await rdev.getProfilePicture(sender)
} catch {
pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
gambar = await getBuffer(pic)
mhan = await rdev.prepareMessage(from, gambar, image, {thumbnail: gambar})
gbutsan = [
...
await rdev.sendMessage(from, gbuttonan, MessageType.buttonsMessage, {
...
if (args.length == 0) return reply(`Namanya?`)
nama = args.join(' ') 
listMsg = {
buttonText: 'Hadiah',
footerText: `*© R - Development*`,
description: `Ciee ${nama}, Dapet hadiah dari Owner...`,
sections: [
{
"title": `Silahlan tekan yah...`,
rows: [
{
"title": "HADIAH :)",
"description": `Hi ${nama}\nHadiah buat kamu nih...`,
"rowId": `${prefix}gg`
},
]}],
listType: 1
}
rdev.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [senderr]},quoted:dev})
break
case 'gg': //testing case 
	if (!isBotGroupAdmins) return reply('Bot gak jadi kasih kamu hadiah\nKarna Bot bukan Admin :(')
	try {
	kik = `${sender}`
	rdev.groupRemove(from, kik)
	rdev.sendMessage(from, 'Awokawok Hadiah :/', MessageType.text, { quoted: dev})
	console.log('Awokawokawok ada yang dapat HADIAH :)') 
	} catch { 
	reply('Chat owner biar dapet SC Gratis :)') 
	}
    break
    case 'delvote':
    ...
    case 'igstory': 
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    rdev.sendMessage(from,link,video,{quoted: dev,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    rdev.sendMessage(from,link,image,{quoted: dev,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
    case 'caripesan':
            ...
            let v = await rdev.searchMessages(q,from,10,1)
            let s = v.messages
            ...
            await rdev.sendMessage(from,'Nih pesannya',text,{quoted:el[i]})
            }
            } catch(e){
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`
            ...
            rdev.sendMessage(from,ram,image,{quoted:dev,caption:rem})
            break
    case 'komiku':
...
anu = await fetchJson(`https://xdev-api.herokuapp.com/muslim/niatshalat`)
    ...
    case 'chara':
            ...
            await rdev.sendMessage(from,li,image,{quoted: dev})
            break
     case 'waifu':
     reply(mess.wait)
waifu = await getBuffer(`https://xdev-api.herokuapp.com/waifu`)
rdev.sendMessage(from, waifu, image, {quoted: ftroli})
break
case 'loli':
reply(mess.wait)
loli = await getBuffer(`https://xdev-api.herokuapp.com/loli`)
rdev.sendMessage(from, loli, image, {quoted: ftroli})
break
case 'sagiri':
reply(mess.wait)
...
rdev.sendMessage(from, tid, image, {quoted: ftroli})
break
case 'quotes':
reply(mess.wait)
...
rdev.sendMessage(from, await getBuffer(tod.result.url), image, {quoted: ftroli})
break
case 'darkjokes':
reply(mess.wait)
					...
					rdev.sendMessage(from, hasil, image, {quoted: ftroli, caption: '*GELAP BOS :V*'})
				break
    case 'pinterest':
            ...
            await rdev.sendMessage(from,di,image,{quoted: dev})
            break
    case 'playstore':
...
rdev.sendMessage(from, huft, image, {quoted: ftroli, caption: teks})
break
case 'google':
...
rdev.sendMessage(from, tid, image, {quoted: ftroli, caption: 'Tebak gambar nya dalam 30 detik'})
setTimeout( () => {
reply(ted)
}, 30000)
break
...
rdev.groupRemove(from, mems_ids)
} else {
rdev.groupRemove(from, [entah[0]])
}
} else {
entah = rdev.message.extendedTextMessage.contextInfo.participant
rdev.groupRemove(from, [entah])
}
break
case 'linkgc':
case 'linkgrup':
...
if (!isGroup) return
if (!isBotGroupAdmins) return reply('Bot not admin')
linkgc = await rdev.groupInviteCode(from)
yeh = `\`\`\`「 Link Group 」\`\`\`\n•> Group Whatsapp :\nhttps://chat.whatsapp.com/${linkgc}\n\n_yo bisa yo ramein_`
rdev.sendMessage(from, yeh, text, { quoted: dev })
break
...
if (!isGroup) return
if (args[0] === 'open') {
rdev.groupSettingChange(from, GroupSettingChange.messageSend, false)
} else if (args[0] === 'close') {
rdev.groupSettingChange(from, GroupSettingChange.messageSend, true)
}
break
...
if (!isBotGroupAdmins) return reply('Bot not admin')
if (!isGroup) return
rdev.groupUpdateDescription(from, `${args.join(" ")}`)
break
...
if (!isBotGroupAdmins) return reply('Bot not admin')
if (!isGroup) return
rdev.groupUpdateSubject(from, `${args.join(" ")}`)
rdev.sendMessage(from, 'Succes change name group', text, {quoted:dev})
break
...
rdev.groupDemoteAdmin(from, [mentionede])
teks = `Members @${mentionede.split('@')[0]} succes demote`
rdev.sendMessage(from, teks, text, {quoted:dev, contextInfo:{mentionedJid:[mentionede]}})
break
...
if (dev.message.extendedTextMessage === undefined || dev.message.extendedTextMessage === null) return reply('Reply members')
mentionede = dev.message.extendedTextMessage.contextInfo.participant
rdev.groupMakeAdmin(from, [mentionede])
...
reply(mess.wait)
owgi = await rdev.downloadMediaMessage(ger)
anu = await upload(owgi)           
titid = await getBuffer(`https://xdev-api.herokuapp.com/canvas/${command}?url=${anu}`)               
rdev.sendMessage(from, titid, image, {quoted : ftroli, caption: '_Success_'}) 
} else {
...
break
    case 'kontag':
            ...
            + 'END:VCARD'.trim()
            rdev.sendMessage(from, {displayName: `${nah}`, vcard: vcard}, contact, {contextInfo: {"mentionedJid": members_ids}})
            break
    case 'sticktag':
            if (!dev.key.fromMe && !isOwner && !isGroupAdmins) return reply(mess.admin)
            ...
            file = await rdev.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
            value = args.join(" ")
            var group = await rdev.groupMetadata(from)
            ...
            rdev.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            } else {
    ...
    case 'totag':
            ...
            ini_buffer = fs.readFileSync(file)
            rdev.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            rdev.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            rdev.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
        } else{
    ...
    case 'fitnah':
            ...
            var bot = gh.split("|")[2];
            rdev.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
            break
    case 'settarget':
    ...
    case 'fitnahpc':
            ...
            const responye = await rdev.sendMessage(jids, `${split[1]}`, MessageType.text, options)
            await rdev.deleteMessage(jids, { id: responye.messageID, remoteJid: jids, fromMe: true })
            break
    case 'tomp3':
            if (!isQuotedVideo) return fakegroup('Reply videonya!')
            fakegroup(mess.wait)
            encmedia = JSON.parse(JSON.stringify(dev).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
            media = await rdev.downloadAndSaveMediaMessage(encmedia)
            ran = getRandom('.mp4')
            exec(`ffmpeg -i ${media} ${ran}`, (err) => {
            fs.unlinkSync(media)
            if (err) return fakegroup(`Err: ${err}`)
            buffer453 = fs.readFileSync(ran)
            rdev.sendMessage(from, buffer453, audio, { mimetype: 'audio/mp4', quoted: dev })
            fs.unlinkSync(ran)
            })
            break
    case 'fast':
            ...
            if (err) return fakegroup(`Err: ${err}`)
            buffer453 = fs.readFileSync(ran)
            rdev.sendMessage(from, buffer453, video, { mimetype: 'video/mp4', quoted: dev })
            fs.unlinkSync(ran)
            })
            break
    case 'slow':
...
const vname = rdev.contacts[i] != undefined ? rdev.contacts[i].vname || rdev.contacts[i].notify : undefined
ini_list.push({
"displayName": 'R - Dev Team',
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname ? `${vname}` : `${rdev.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
hehe = await rdev.sendMessage(from, {
"displayName": `${ini_list.length} kontak`,
"contacts": ini_list 
}, 'contactsArrayMessage', { quoted: dev })
rdev.sendMessage(from,'Nih Kak Creator R - Dev Team',text,{quoted: hehe})
break
case 'style':
      ...
      let hasil = `*Hasil* :\n${res.data.result}`;
      rdev.sendMessage(from, hasil, MessageType.text, { quoted: dev});
    })
			break
case 'pastebin':
if(!q) return reply(`Example : R - Dev Bot`)
anu = await fetchJson(`https://api-anoncybfakeplayer.herokuapp.com/pastebin?text=${q}`, {method: 'get'})
                   rdev.sendMessage(from, `${anu.result}`, text, {quoted: ftroli})
                     break
    case 'anime':
            reply(mess.wait)
            ...
            rdev.sendMessage(from,media,image,{quoted:dev,caption:'NIH'})
            }
            )
            .catch((error) => {
            console.log(error); 
    ...
    case 'kontak':
            ...
            + `TEL;type=CELL;type=VOICE;waid=${entah}:${phoneNum('+' + entah).getNumber('internasional')}\n`
            + 'END:VCARD'.trim()
            rdev.sendMessage(from, {displayName: `${nah}`, vcard: vcard}, contact)
            break    
    case 'take':
    case 'colong':
			...
			require('./lib/fetcher.js').modStick(media, rdev, dev, from)
			break
	case 'stikerwm':
	case 'stickerwm':
    case 'swm':
            ...
            .on('error', (e) => {
            console.log(e)
            rdev.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: dev })
            fs.unlinkSync(media)
            })
            .on('end', () => {
            _out = getRandom('.webp')
            ...
            .on('exit', () => {
            rdev.sendMessage(from, fs.readFileSync(_out),'stickerMessage', { quoted: dev })
            fs.unlinkSync(out)
            fs.unlinkSync(_out)
            fs.unlinkSync(media)
            })
            ...
            const media = await rdev.downloadAndSaveMediaMessage(encmedia)
            pe = args.join('')
            var a = pe.split("|")[0];
            var b = pe.split("|")[1];
            await createExif(a,b)
    ...
    case 'upswteks':
            ...
            rdev.sendMessage('status@broadcast', `${q}`, extendedText)
    ...
    case 'upswimage':
            ...
            cihcih = await rdev.downloadMediaMessage(swsw)
            rdev.sendMessage('status@broadcast', cihcih, image, { caption: `${q}` })
            bur = `Sukses Upload Story Image dengan Caption: ${q}`
            rdev.sendMessage(from, bur, text, { quoted: dev })
            } else {
    ...
    case 'upswvideo':
            ...
            cihcih = await rdev.downloadMediaMessage(swsw)
            rdev.sendMessage('status@broadcast', cihcih, video, { caption: `${q}` }) 
    ...
    case 'fdeface':
            ...
            const tipes = await rdev.downloadAndSaveMediaMessage(dipes)        
    		...
    		var mat = await rdev.generateLinkPreview(url)
    		mat.title = title;
    		mat.description = desc;
    		mat.jpegThumbnail = bufer;
   			mat.canonicalUrl = buu; 
    		rdev.sendMessage(from, mat, MessageType.extendedText, anu)
            break
    case 'public':
          	if (!dev.key.fromMe && !isOwner) return
          	...
          	banChats = false
          	fakestatus(`Succes Change To ${command}`)
          	break
	case 'self':
          	if (!dev.key.fromMe && !isOwner) return
      ...
      //********** ADD **********//
		...
		if (!dev.key.fromMe && !isOwner) return reply (mess.owner)
				...
				delb = await rdev.downloadMediaMessage(boij)
				setiker.push(`${svst}`)
				fs.writeFileSync(`./temp/stick/${svst}.webp`, delb)
				fs.writeFileSync('./temp/stick.json', JSON.stringify(setiker))
				rdev.sendMessage(from, `Sukses Menambahkan Sticker\nCek dengan cara ${prefix}liststik`, MessageType.text, { quoted: dev})
				break
					case 'dellstik':
				...
				rdev.sendMessage(from, teks.trim(), extendedText, { quoted: dev, contextInfo: { "mentionedJid": setiker } })
				break
				...
				rdev.sendMessage(from, teks.trim(), extendedText, { quoted: dev, contextInfo: { "mentionedJid": imagenye } })
				break
			...
			case 'vnlist':
				...
				rdev.sendMessage(from, teks.trim(), extendedText, { quoted: dev, contextInfo: { "mentionedJid": audionye } })
				break
				...
				rdev.sendMessage(from, result, sticker,{quoted:dev})
				} catch {
				...
				try {
				buffer = fs.readFileSync(`./temp/foto/${namastc}.jpeg`)
				rdev.sendMessage(from, buffer, image, { quoted: dev, caption: `Result From Database : ${namastc}.jpeg` })
				} catch {
				  reply('Pack tidak terdaftar')
				}
				break
				...
				try {
				buffer = fs.readFileSync(`./temp/video/${namastc}.mp4`)
				rdev.sendMessage(from, buffer, video, { quoted: dev, caption: `Result From Database : ${namastc}.mp4` })
				} catch {
				  reply('Pack tidak terdaftar')
				}
				break
...
break
 	case 'hidetag':
			...
			rdev.sendMessage(from, optionshidetag, text)
			break
...
rdev.updatePresence(from, Presence.composing)
					...
					if (isMedia && !dev.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							...
							rdev.sendMessage(_.jid, buff, video, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 0000000000, status: 200, thumbnail: fs.readFileSync('./stik/thumb.jpeg'), surface: 200, message: `Broadcast`, orderTitle: `Broadcast`, sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `*BROADCAST BOT*\n\n${body.slice(4)}` })
						...
						for (let _ of anu) {
							...
							rdev.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 0000000000, status: 200, thumbnail: fs.readFileSync('./stik/thumb.jpeg'), surface: 200, message: `Broadcast`, orderTitle: `Broadcast`, sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `*BROADCAST BOT*\n\n${body.slice(4)}` })
    ...
    case 's':
            if (isImage || isQuotedImage) {
                    ...
                    let media = await rdev.downloadAndSaveMediaMessage(encmedia, `./stik/${sender}`)
                    await ffmpeg(`${media}`)
							.input(media)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./stik/data.exif ./stik/${sender}.webp -o ./stik/${sender}.webp`, async (error) => {
									...
									rdev.sendMessage(from, fs.readFileSync(`./stik/${sender}.webp`), sticker, {quoted: dev})
...
break
	...
	case 'ytsearch':
        	...
        	return await rdev.sendMessage(from, 'Error!', MessageType.text, dload)
    		}
    		aramat = aramas.all 
    ...
    case 'image':
            ...
            rdev.sendMessage(from,{url:images},image,{quoted:dev})
            });
            break
 	case 'tiktok':
		...
		hx.ttdownloader(`${args[0]}`)
    		...
    		.then(async (a) => {
    		me = `*Link* : ${a.data}`
		rdev.sendMessage(from,{url:`${nowm}`},video,{mimetype:'video/mp4',quoted:dev,caption:me})
		})
		})
    ...
    case 'brainly':
			...
			rdev.sendMessage(from, teks, text,{quoted:dev,detectLinks: false})                        
            })              
			break
    case 'ig':
    ...
    if (!dev.key.fromMe && !isOwner) return reply(mess.owner)
			if (!q) return fakegroup(mess.wrongFormat)
			exec(q, (err, stdout) => {
			if (err) return fakegroup(`R - Dev Team:~\n${err}`)
			if (stdout) {
			fakegroup(stdout)
			}
			})
    ...
    if (!dev.key.fromMe && !isOwner) return reply(mess.owner)
            ...
            var response = await rdev.acceptInvite(codeInvite)
            fakestatus('SUKSES')
            } catch {
    ...
    case 'test':
            run = process.uptime() 
            teks = `${kyun(run)}`
            rdev.sendMessage(from, teks, text, {quoted: fgc})
            break  
	case 'speed':
	case 'ping':
			...
			rdev.sendMessage(from, pingnya, text, {quoted: ftroli})
			})
			break  
    case 'totag':
    if (!dev.key.fromMe && !isOwner) return reply(mess.owner)
    ...
    case 'tomp4':
            ...
            owgi = await rdev.downloadAndSaveMediaMessage(ger)
    ...
    case 'tourl':
            ...
            owgi = await rdev.downloadMediaMessage(boij)
    ...
    rdev.sendMessage(from, tod, image, {quoted: ftroli, caption: 'Awas ketahuan kak :v'})
    break
    case 'inspect':
            try {
            if (!isUrl(args[0]) && !args[0].includes('whatsapp.com')) return reply(mess.Iv)
            ...
            jids = []
            let { id, owner, subject, subjectOwner, desc, descId, participants, size, descOwner, descTime, creation} = await rdev.query({ 
...
${descOwner ? `*Desc diubah oleh* : @${descOwner.split('@')[0]}` : '*Desc diubah oleh* : -'}\n*Tanggal* : ${descTime ? `${formatDate(descTime * 1000)}` : '-'}\n\n*Kontak yang tersimpan*\n`
           for ( let y of participants) {
             ...
             rdev.sendMessage(from,par,text,{quoted:dev,contextInfo:{mentionedJid:jids}})
             } catch {
             reply('Link error')
             }
             break
            ...
            case 'setppbot':
			    rdev.updatePresence(from, Presence.composing)
					...
					media = await rdev.downloadAndSaveMediaMessage(enmedia)
					await rdev.updateProfilePicture(botNumber, media)
					reply('Makasih profil barunya😗')
					break
					
										case 'sider':
infom = await rdev.messageInfo(from, dev.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
shape = '▢'
...
rdev.sendMessage(from,'Nih Kak Creator R - Dev Bot',text,{quoted: hehe})
}
if (button == 'GITHUB') {
console.log('GITHUB')
rdev.sendMessage(from, `*Nih kak github R - Dev Team*\n*Scnya free kak dan juga free api 🤗*\n*Ingat ! jangan diperjual belikan yah !*\n\nhttps://github.com/RyuuOfficial`, text, {quoted: dev})
}
if (button == 'PROMO') {
console.log('PROMO')
rdev.sendMessage(from, `Jika ingin membeli *Script*
Bisa hubungi *Owner* kami
wa.me/6282316153393
...
*Info Script* :
Bisa di Run Selama-lamanya
Tanpa update server Baileys`, text, {"contextInfo": {text: 'HelloWorld',"forwardingScore": 3,isForwarded: true,sendEphemeral: true,mentionedJid: [sender],"externalAdReply": {"title": `Hallo kak ${pushname}`,"body": ``,"previewType": "PHOTO","thumbnailUrl": `https://telegra.ph/file/a082de791ef8aff96ec24.jpg`,"thumbnail": "","sourceUrl": ""}}, quoted: dev})
}
if (budy.startsWith('$')){
if (!dev.key.fromMe && !isOwner) return
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`R - Dev Team:~ ${err}`)
if (stdout) {
reply(stdout)
}
})
...
if (!dev.key.fromMe && !isOwner) return
try {
return rdev.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: dev})
} catch(err) {
e = String(err)
reply(e)
}
...
console.log('\x1b[1;37m>', '[', '\x1b[1;32mR-Dev\x1b[1;37m', ']', time, color(">", "green"), 'from', color(sender.split('@')[0]), 'args :', color(args.length))
} catch(e){
reply(String(e))
}
}
